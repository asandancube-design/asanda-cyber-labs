# Cybersecurity Portfolio

Hi, I’m Philisiwe 👋  
I’m an aspiring SOC Analyst building hands-on cybersecurity skills through labs, incident simulations, and automation projects.

This repository showcases my learning journey as I develop practical experience in cybersecurity, focusing on threat detection, log analysis, and security automation.

## 🔍 Focus Areas
- SOC Analysis & Incident Response  
- Threat Detection & Log Analysis  
- Security Automation (Python basics)  
- AI in Cybersecurity awareness  

## 📁 What You’ll Find Here
- Phishing analysis write-ups  
- Incident response simulations  
- Log analysis scripts  
- Basic security automation projects  
- Cybersecurity + AI learning notes  

## 🚀 Goal
To grow into a SOC Analyst who understands both traditional cybersecurity and emerging AI-driven threats.

## 🧠 Learning Approach
I focus on practical, hands-on learning through real scenarios, not just theory.

---

Feel free to explore my work 👇
